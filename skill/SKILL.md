...
Use `lossline` if it is on PATH. Otherwise run it without installing:

```bash
uvx --from "git+https://github.com/bednarjosef/lossline#subdirectory=python" lossline ...
```

Once the package is on PyPI, `uvx lossline ...` and `pip install lossline` work too.
...
| `lossline ls <project>` | list runs: status, step, age, and key metrics |
| `lossline show <project>/<run>` | config, machine, and per-metric last / min / max / trend / sparkline |
| `lossline compare <p>/<a> <p>/<b> ...` | put runs side by side: differing config keys, last value of each metric, `*` on the best |
...
| `lossline export <project>/<run> [--format csv\|jsonl]` | dump every row |

`ls`, `show` and `compare` take `-m 'eval/*'` to filter metrics (repeatable, globs) and
`--json` for machine-readable output. Prefer the plain text: it is already compact and
reads better than JSON.
...
```bash
lossline wait seqmem/latest                              # until it finishes, fails or stalls
...
1. Install on the box: `pip install "git+https://github.com/bednarjosef/lossline#subdirectory=python"`
   (or `pip install lossline` once it's published).
...
3. The logger prints `lossline: run <project>/<id> (...)` to stderr at start. Note that
   reference, or use `<project>/latest`.
